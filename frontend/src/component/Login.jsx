...
const Login = () => {
    ...
    return (
        <>
            {/* Adjusted non-standard w-87.5 to an arbitrary or standard tailwind width */}
            <form
                onSubmit={handleSubmit}
                className="w-full sm:w-[350px] text-center bg-white/5 border border-white/10 rounded-2xl px-8 mx-auto mt-20">
                ...
                {state !== "login" && (
                    <div className="flex items-center mt-6 w-full bg-white/5 ring-2 ring-white/10 focus-within:ring-indigo-500/60 h-12 rounded-full overflow-hidden pl-6 gap-2 transition-all ">
                        ...
                        {/* Fixed: required is now conditional */}
                        <input type="text" name="name" placeholder="Name" className="w-full bg-transparent text-white placeholder-white/60 border-none outline-none " value={formData.name} onChange={handleChange} required={state !== "login"} />
                    </div>
                )}
                ...
                <button type="submit" className="mt-6 w-full h-11 rounded-full text-white bg-indigo-600 hover:bg-indigo-500 transition " >
                    {state === "login" ? "Login" : "Sign up"}
                </button>

                {/* Fixed: Moved onClick exclusively to the interactive text span */}
                <p className="text-gray-400 text-sm mt-3 mb-11" >
                    {state === "login" ? "Don't have an account?" : "Already have an account?"}
                    <span 
                        onClick={() => setState(prev => prev === "login" ? "register" : "login")} 
                        className="text-indigo-400 hover:underline ml-1 cursor-pointer select-none"
                    >
                        click here
                    </span>
                </p>
            ...
            {/* Soft Backdrop - Adjusted to arbitrary brackets for non-standard sizing */}
